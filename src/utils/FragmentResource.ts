...
class ClientEdgeQueryResultsCache {
	...
	get(fragmentIdentifier: string): undefined | Array<QueryResult> {
	...
	recordQueryResults(
		...
		if (!existing) {
			...
			for (const queryResult of value) {
	...
	subscribe(fragmentResult: FragmentResult, callback: () => void): Disposable {
		...
		if (RelayFeatureFlags.ENABLE_CLIENT_EDGES) {
			...
			if (clientEdgeQueryResults?.length) {
				...
				for (const queryResult of clientEdgeQueryResults) {
