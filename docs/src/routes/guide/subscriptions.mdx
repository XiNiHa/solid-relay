...
function PostComments(props: { postId: string }) {
	...
	const data = createLazyLoadQuery<PostCommentsQuery>(PostCommentsQuery, () => ({
		postId: props.postId,
	}));
		...
		updater: (store, data) => {
			if (data?.commentAdded) {
				...
				if (post) {
					...
					const connection = ConnectionHandler.getConnection(post, "PostComments_comments");
	...
	return (
		<div>
			<For each={data()?.post.comments.edges}>{(edge) => <Comment $comment={edge.node} />}</For>
		</div>
	);
}
```
