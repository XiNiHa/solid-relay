	...
	return (
		<div>
			<Show when={query.hasPrevious}>
				<button onClick={handleLoadPrevious} disabled={query.isLoadingPrevious}>
			...
			<Show when={query.hasNext}>
				<button onClick={handleLoadMore} disabled={query.isLoadingNext}>
	...
	return (
		<div>
			<div>
				...
				<button onClick={() => query.loadNext(5)} disabled={!query.hasNext || query.isLoadingNext}>
					Next →
				</button>
			</div>
				...
				<For each={query()?.posts.edges}>
					{(edge) => (
						<article>
							...
							<small>
								By {edge.node.author.name} on {edge.node.createdAt}
							</small>
						</article>
					)}
				</For>
...
function InfiniteUserList(props: { $query: UserList_query$key }) {
	...
	return (
		<div>
			<div>
				<For each={query()?.users.edges}>{(edge) => <UserCard $user={edge.node} />}</For>
			</div>

			<Show when={query.hasNext}>
				<div ref={loadMoreRef}>
					<Show when={query.isLoadingNext} fallback={<div>Scroll to load more...</div>}>
...
function SearchResults(props: { $query: SearchResults_query$key }) {
	...
	const query = createPaginationFragment(
		graphql`
			...
			@refetchable(queryName: "SearchResultsPaginationQuery") {
				searchUsers(query: $searchTerm, first: $count, after: $cursor)
	...
	return (
		<div>
			...
			<div>
				<For each={query()?.searchUsers.edges}>{(edge) => <UserCard $user={edge.node} />}</For>
			</div>

			<Show when={query.hasNext}>
				<button onClick={() => query.loadNext(10)} disabled={query.isLoadingNext}>
...
function VirtualizedUserList(props: { $query: UserList_query$key }) {
	const query = createPaginationFragment(
		graphql`
			fragment UserList_query on Query @refetchable(queryName: "UserListPaginationQuery") {
				users(first: $count, after: $cursor) @connection(key: "UserList_users") {
					edges {
						node {
							id
