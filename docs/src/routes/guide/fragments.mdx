...
function UserList() {
	...
	return (
		<Show when={data()}>
			{(data) => (
				<div>
					<For each={data().users}>{(user) => <UserCard $user={user} />}</For>
				</div>
			)}
		</Show>
	);
...
function UserProfile(props: { $user: UserProfile_user$key }) {
	const [user, refetch] = createRefetchableFragment(
		graphql`
			fragment UserProfile_user on User @refetchable(queryName: "UserProfileRefetchQuery") {
				id
				name
				email
				posts(first: 5) {
		...
		<Show when={user()}>
			{(user) => (
				<div>
					...
					<div>
						<For each={user().posts.edges}>{(edge) => <div>{edge.node.title}</div>}</For>
					</div>
				</div>
			)}
		</Show>
	...
	const data = createPaginationFragment(
		graphql`
			...
			@refetchable(queryName: "UserPostsPaginationQuery") {
				posts(first: $count, after: $cursor) @connection(key: "UserPosts_posts") {
					edges {
						node {
							id
							title
			...
			{(user) => (
				<div>
					...
					<Show when={data.hasNext}>
						<button onClick={handleLoadMore} disabled={data.isLoadingNext}>
...
const UserQuery = graphql`
	query UserQuery($userId: ID!, $showContent: Boolean!) {
		user(id: $userId) {
			...UserPosts_user @arguments(count: 10, includeContent: $showContent)
		}
	}
`;
```
